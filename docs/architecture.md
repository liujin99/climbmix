...
| **kmeans**（细簇聚类） | 1000 个细簇 = 聚类**基底**；只依赖池内容，与用户旋钮无关 | run_experiment Stage 1 | 基底 npz 缺失时（跑完永久入账） | ~20min |
...
设计指令（用户裁决）：**本地盘只放可再生暂存**——上量后本地必爆，所以块文件
收官即清（`rm -f <key>/block_*.npy <key>/manifest.json`），常驻只剩 ~1.5GB
派生缓存；换池时 40min 重 merge 再生性能层。
...
```
 ...
 ③ 现算（进入 discovery）:
    ...
    3b. kmeans_K1000.npz 存在?              ◀ 细簇基底，与旋钮无关
        │是 → 载入标签（39min 免付）
        │否 → prescan（块级并行 ~3min）→ 训练 → 全池指派 → 写 npz（~20min）
...
|---|---|---|
| 旋钮 + 代码都没变 | ② 命中 | 秒级 |
| 旋钮变了（K / 阈值）或代码变了 | ③b 命中 → ③c | merge 段 ~2min |
| 池变了 / 本地块被清 | 先跑 preprocess（40min）再走 ③ 全程 | ~1h |
...
| 场景 | 路径 | 发射 → 首批派发 |
|---|---|---|
| 旋钮 + 代码不变（常规重发） | ② 命中 | **~20min**（爬坡主导） |
| 代码变更后首发 | ③b 命中 + merge 段 | ~22min |
| 本地块被清（收官清理后） | preprocess + ②③b | +40min |
