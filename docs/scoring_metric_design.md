...
2. SNR 权重
   chance   = BENCHMARK_CHANCE[b]                 # MC=0.25, CoT≈0 (types.py)
   p̂        = chance + (1-chance)·mean(acc)       # 经验平均正确率 (raw 单位)
   σ²_noise  = p̂(1-p̂) / (K·(1-chance)²)          # 二项噪声, 换算回 centered 单位
   ...
   # ((raw - chance)/(1-chance)), 噪声项必须同除 (1-chance)², 否则噪声
   # 低估 1/(1-chance)² 倍、w 偏高 (prod2 实测: mmlu_stem f +0.195 → -0.43)。
   # 单位注意 (2026-09-15 修复): chance 必须按任务类型取 — CoT 任务
   # centered==raw, 套用 MC 的 0.75² 会高估噪声 1.78×; 加上 worst-case
   # p=0.25 对 p̂≈0.02 的 gsm8k 共高估 ~20× (详见 5.3)。
...
**以上推理被 prod3 (2026-09-15) 证伪，本设计已废弃**：

- 论据 4 的假设"accuracy 几乎全噪声"在 config-间方差层面**不成立**：prod3 的 54 个实测点上，gsm8k 的 σ²_between = 1.14e-4，是真实二项噪声 (1.7e-5) 的 **6.7 倍** —— f_true = +0.85，是全部 6 个任务里**最强的信号**；而 0.25/K 地板（3.37e-4，高估 ~20×）把它判为 f=-1.97、w=0，**最强信号被当作噪声丢弃**。
- 论据 1 的"过度保守的后果只是多用 NLL"**不是良性**：NLL 是有偏信号（偏好可预测文本），w=0 后该任务完全由 NLL 排序。prod3 终轮 4/6 任务 w=0 → 目标静默翻转为纯 NLL 排名（score 与 mean(nll_z) 的 r=-0.95）→ 搜索选出百科/散文簇（最可预测）→ winner 在 d28@3B 把 gsm8k 从 0.119 砍到 0.052。
- 新方案：**经验 p̂ + 按任务 chance 语义换算 + w_floor=0.5 兜底**。p̂ 用 config 间均值（估计噪声 ~10-2 量级，远小于信号判断所需的区分度）；极端 p̂≈0 的高估风险由 w_floor=0.5 封顶（最多损失一半 accuracy 话语权，不会全部让给 NLL）。

### 5.4 w_floor 的选择（0.5，prod3 后修订）

| w_floor | 效果 | 适用场景 |
|---------|------|---------|
| 0.0 | hard benchmark 可达 w=0 (纯 NLL) | ~~默认~~ prod3 事故形态 |
| **0.5** | **hard benchmark 至少 50/50** | **默认 (2026-09-15 起)** |

prod3 (2026-09-15) 终轮实测：六任务里四个 w=0（gpqa/gsm8k/math 显式为 0，mmlu 0.021），目标 = ~纯 NLL 排名。原论证"如果 accuracy 确实是噪声，w 应该是 0，不应该人为设下限"隐含假设 NLL 是中性兜底——**它不是**：NLL 系统性偏好可预测文本（百科/散文/题面），而这类 mixture 在目标尺度会饿死生成式推理能力。w_floor=0.5 的语义：**NLL 最多只能"补充"（分走一半话语权），永远不能"接管"** —— 与 acc-primary 的设计意图一致。另有每轮打印的 [Scoring] WARNING（acc share + 噪声绑定任务清单），目标构成再也不静默。
...
**prod3 (2026-09-15, N=54) 实测 vs 上表预测 —— 两个方向都错了**：

| Benchmark | 预测 σ²_between | 实测 σ²_between | 预测 w | 旧式实际 w | 修正后 w | 修正后主导 |
|-----------|----------------|----------------|--------|-----------|----------|-----------|
| arc_easy | ~0.005 | 1.25e-4 (40× 低估) | ~0.99 | 0.249 | 0.35 | NLL 偏 |
| arc_challenge | ~0.003 | 3.11e-4 (10× 低估) | ~0.97 | 0.390 | 0.43 | NLL 偏 |
| mmlu_stem | ~0.004 | 6.4e-5 (60× 低估) | ~0.99 | 0.021 | 0.22 | NLL 偏 |
| gpqa_diamond | ~0.0003 | 9.13e-4 | ~0.0 | 0.000 | 0.10 | NLL |
| gsm8k_cot | ~0.00002 | 1.14e-4 (**5.7× 高估了噪声**) | ~0.0 | 0.000 | **0.93** | **accuracy** |
| math_cot_500 | ~0 | 3.5e-6 | ~0.0 | 0.000 | 0.39 | NLL 偏 |

- 设计期假设"简单任务 between 大 → accuracy 主导"落空：d20 上 mixture 对 MC 任务的真实影响比预测小 1-1.5 个数量级 → 全部贴地。
- 设计期假设"gsm8k 全噪声"也错：mixture 对 gsm8k 的影响比预测大 5.7 倍，是**全场最强信号**——被 0.25/K 地板（高估 ~20×）判废。
- 教训：**σ²_between 只能用实测估计，不能从 base-model 分数外推**；第一轮数据落地后必须回看 w 表（现在每轮打印）。
