...
- **流式路径冒烟全通过(2026-08-31, 20 分片 / 2,319,341 docs)**:
  首跑在 40% 处 8 worker 全灭——np.array_split 派给 worker 的
  分片号是 np.int64,首笔账本 json.dump 抛 TypeError(except
  OSError 接不住);b96235d 双层修复(父进程传 int + 账本写入
  防御性强转),且账本写是 tmp+rename 原子,崩溃只留不可见
  .tmp,重跑零清理。修复后全链路验收:8 worker 均衡(2-3
  分片/卡),峰值 **822 docs/s**(爬坡 ~5 min,尾部随 worker
  收工缓降至 ~777,500K 采样路径 750 预测被证实);Validate
  全绿(NaN/Inf/zero-norm/off-norm=0,全范数 1.0000);分块
  聚类路径(chunk=2.1M)407s 落 3-7 min 窗;全量 clusters.csv
  落盘;全程 4127s。全量标签 vs 500K 采样:尾部弱簇更弱
  (簇均 min 2.74 vs 3.91),t=3.0 仍只剪 2/1000 簇(0.2%
  docs)——安全网定位不变。断点续跑(Ctrl-C 重跑)未验,非
  阻塞。子集缓存键与全池键不同,零污染,测完可删。
