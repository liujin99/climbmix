...
to algorithm concepts (domain/cluster, quality, text, char_count).

Ported from quadmix.data.dataset_schema. Decouples climbmix from any
specific dataset schema so that arbitrary datasets can be run with zero
code changes — just write a YAML config.

Usage:
    # From YAML file (primary entry point)
    ...
    schema.quality_cols       # ["stem_relevance", ...]
    schema.quality_directions  # [True, True, ..., True]
    schema.text_col           # "text"
    schema.char_count_col     # "char_count_col" or None

    # Prune threshold (optional, in YAML)
    schema.prune_threshold     # 3.0

DatasetSchema() 无参构造会报错 — 必须通过 from_yaml() 加载配置。
示例配置: config/schema_stem.yaml
"""
...
class DatasetSchema:
    ...
    def __post_init__(self):
        if self.domain_col is None and self.quality_cols is None and self.text_col is None:
            raise ValueError(
                ...
                "示例: schema = DatasetSchema.from_yaml('config/schema_stem.yaml')\n"
                "详见 config/ 目录下的示例 YAML 配置。"
            )
    ...
    def from_yaml(cls, path: str) -> "DatasetSchema":
        ...
        return cls(
            ...
            prune_threshold=float(data.get("prune_threshold", 3.0)),
            preprocessed_pattern=data.get("preprocessed_pattern", "*.parquet"),
        )
    ...
    def _validate(
        ...
        for col in required_cols:
            if col not in parquet_columns:
                ...
                if not dt.startswith(("float", "double", "Float64", "Float32", "int", "Int64", "Int32")):
                    dtype_mismatch.append((col, dt, "float/int"))
