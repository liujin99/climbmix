...
try:
    ...
    def _memory_efficient_attention(q, k, v, attn_bias=None, p=0.0, **kw):
        ...
        if isinstance(attn_bias, _BlockDiagonalMask):
            ...
            q_f = q_pad.transpose(1, 2).bfloat16()  # (n, H, max_s, D)
            k_f = k_pad.transpose(1, 2).bfloat16()  # (n, H, max_ks, D)
    ...
    if device == "npu":
        ...
        if actual_device == "npu":
            try:
                ...
                print(f"[Embed] Model loaded in {time.time() - t0:.1f}s (bf16+manual_attn, msl=512, bs={batch_size})")
...
def _embed_streaming_worker(worker_id, shard_indices, shard_infos, text_col,
    ...
    model.bfloat16()
    print(f"[NPU {worker_id}] Model loaded (bf16+manual_attn, max_seq_len={truncate_len})", flush=True)
