...
#
#  用法: 编辑下方 EDIT 块 → bash runs/run_extend_experiment.sh
#        干跑: LAUNCH=0 bash runs/run_extend_experiment.sh (校验+注入+打印, 不发射)
#  · 注入只发生一次 (新 run 无 search_state 时); 中断后重跑本命令 = 续跑
#  · 其余发射参数 (NPU_PER_EXP/REMOTE_*/...) 在 runs/run_experiment.sh 的 EDIT 块
# ═══════════════════════════════════════════════════════════════════
set -euo pipefail
...
# 传给 runs/run_experiment.sh (其余发射参数在它的 EDIT 块):
...
exec bash runs/run_experiment.sh
