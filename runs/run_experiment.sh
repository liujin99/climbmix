#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════
#  ClimbMix 实验入口 — 完整链 (单脚本):
#  池 → 聚类 → d20 搜索 → 混料 → 双臂 (climb/random) 训练评测 → 报告
#
#  用法: 编辑下方 EDIT 块 → bash runs/run_experiment.sh
#        后台: nohup bash runs/run_experiment.sh > run.log 2>&1 & (建议配 setsid)
#        干跑: LAUNCH=0 bash runs/run_experiment.sh (校验+打印, 不启动)
#        全 env 直跑: EXP_NAME=myexp bash runs/run_experiment.sh
#        (输出 result/myexp_current; EDIT 块 = 默认值面, env 可覆盖一切)
#
...
#  停止: pkill -f run_experiment.sh —— 后台启动(自有进程组, 建议配 setsid)时
...
#
#  想复用已有实验的 d20 已测点做增量实验? → runs/run_extend_experiment.sh
#  入口家族 (后三个 extend_* = 对已有实验的扩展动作, 非必经下一站):
#    run_experiment(本) / run_extend_experiment / run_extend_traineval /
#    run_extend_eval
#  (2026-09-20: 原 runs/run_climbmix.sh 主管道引擎并入本脚本 — 单入口单语义)
# ═══════════════════════════════════════════════════════════════════════
...
# `pkill -f run_experiment.sh` cannot strand NPU-holding grandchildren (the
...
# ── Legacy-knob guards (发射最早期拦截, 防静默指纹漂移; 派生块见下方引擎体) ──
# TARGET_STEPS / PROXY_NUM_ITERATIONS 不再是用户旋钮: 步数由 token 预算派生
...
if [ "$LAUNCH" != "1" ]; then
    echo
    echo "[dry-run] 将执行: 主管线 Steps 1-8 (env 如上)"
    exit 0
fi
...
#  默认值面: env 覆盖 > 下方默认; EDIT 块在文件头部 (操作面)。
# ═══════════════════════════════════════════════════════════════════════
...
# TARGET_STEPS 不再是用户旋钮 (守卫已上移至发射头部)。
...
# 外部设置 = 遗留配置, 就地报错 (守卫已上移至发射头部); 需要更强/更省的搜索信号时改预算即可
...
# 生产若想升 dbs: 先小规模实测 "Peak memory usage" 余量 (历史实证: docs/nan_investigation.md)。
...
# ── NPU Environment (deliberately minimal — matches proven runs/infra/train_base_model.sh) ──
# 历史: 这里的分配器覆盖块 (memory_pool:True + PYTORCH_NPU_ALLOC_MAX_SIZE=60G
# + friends) 曾在 torch_npu 的 allocator 记账之外填满 device 0
# → kernel-load OOMs (aclnnMean 207001
